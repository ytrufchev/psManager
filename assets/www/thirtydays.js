...
$.ajax(settings).done(function (response) {
  ...
  "data": 'type=Android&interval=3&token='+localStorage.getItem("token")
}
$.ajax(settings).done(function (response) { 
...
var settings = {
  ...
  "data": 'type=clicks&period=current_month&token='+localStorage.getItem("token")
}
$.ajax(settings).done(function (response) {
  ...
  clickDataPoints[parseInt(res)] = (clickDataPoints[parseInt(res)] + 1);
  }
});
...
for(i in clearedArray1){
  ...
  var days = utc.getDate();
  for(i=0; i <= days; i++){
    commissionDataLabels[i] = "";
    commissionDataPoints[i] = 0;
  }
for(i in commissions){
  ...
  commissionDataPoints[parseInt(day)] = (commissionDataPoints[parseInt(day)] + amount);
}
});
...
for(i in conversions){
  ...
  var date = new Date(timestamp);
  var day = date.getDate();
  conversionDataPoints[parseInt(day)] = (conversionDataPoints[parseInt(day)] + amount);
}
