...
$.ajax(settings).done(function (response) {
  ...
  "data": 'type=Android&interval=0&period=today&token='+localStorage.getItem("token")
}
$.ajax(settings).done(function (response) { 
...
var settings = {
  ...
  "data": 'type=clicks&period=today&token='+localStorage.getItem("token")
}
$.ajax(settings).done(function (response) {
  ...
  var time1 = str.match(timePatt);
  var hourPatt = /^\d+/gmi;
  var hour = time1.toString();
  var hourNew = hour.match(hourPatt);
  clickDataPoints[hourNew] = (clickDataPoints[hourNew] + 1);
  }
});
...
var data  = {
    labels: window.clickDataLabels,
    display: false,
    datasets: [{
            backgroundColor: gradient,
            borderWidth: 1,
            borderColor: '#1459e3',
            data: window.clickDataPoints,
            pointRadius: 0
    }]
};
...
for(i in commissions){
  ...
  var date = new Date(((timestamp/1000)-10800)*1000);
  var hour = date.getHours();
  commissionDataPoints[parseInt(hour)] = (commissionDataPoints[parseInt(hour)] + amount);
}
});
...
for(i in conversions){
  ...
  var date = new Date(((timestamp/1000)-10800)*1000);
  var hour = date.getHours();
  conversionDataPoints[parseInt(hour)] = (conversionDataPoints[parseInt(hour)] + amount);
}
